...
Font Load(FontKey key, TArgs&&... constructor_args) {
	return GetManagers().font.Load(
		key, std::forward<TArgs>(constructor_args)...
	);
}
...
template <
	typename... TArgs, type_traits::constructible<Texture, TArgs...> = true>
Texture Load(TextureKey key, TArgs&&... constructor_args) {
	return GetManagers().texture.Load(
		key, std::forward<TArgs>(constructor_args)...
	);
}
...
template <
	typename... TArgs, type_traits::constructible<Shader, TArgs...> = true>
Shader Load(ShaderKey key, TArgs&&... constructor_args) {
	return GetManagers().shader.Load(
		key, std::forward<TArgs>(constructor_args)...
	);
}
...
void SetStartScene(TArgs&&... constructor_args) {
	PTGN_ASSERT(
		!scene::Has(start_scene_key), "Cannot load more than one start scene"
	);
	...
	GetManagers().scene.Load(
		start_scene_key,
		std::make_shared<T>(std::forward<TArgs>(constructor_args)...)
	);
}
...
std::shared_ptr<T> Load(SceneKey key, TArgs&&... constructor_args) {
	PTGN_CHECK(
		key != impl::start_scene_key,
		"Cannot load scene with key == 0, it is reserved for the starting scene"
	);
	return std::static_pointer_cast<T>(GetManagers().scene.Load(
