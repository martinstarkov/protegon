...
void DrawThickLine(
	int x1, int y1, int x2, int y2, double pixel_thickness, const Color& color
);
void DrawCapsule(int x1, int y1, int x2, int y2, int r, const Color& color);
void DrawSolidCapsule(
	int x1, int y1, int x2, int y2, int r, const Color& color
);
void DrawThickCapsule(
	int x1, int y1, int x2, int y2, int r, double pixel_thickness,
	const Color& color
);
void DrawVerticalLine(int x, int y1, int y2, const Color& color);
void DrawThickVerticalLine(
	int x, int y1, int y2, double pixel_thickness, const Color& color
);
void DrawHorizontalLine(int x1, int x2, int y, const Color& color);
void DrawThickHorizontalLine(
	int x1, int x2, int y, double pixel_thickness, const Color& color
);
...
void DrawCapsuleImpl(
	SDL_Renderer* renderer, int x1, int y1, int x2, int y2, int r
);
void DrawSolidCapsuleImpl(
	SDL_Renderer* renderer, int x1, int y1, int x2, int y2, int r
);
...
void DrawXPerpendicular(
	SDL_Renderer* renderer, int x1, int y1, int dx, int dy, int xstep,
	int ystep, int einit, int w_left, int w_right, int winit
);
...
void DrawXThickLine(
	SDL_Renderer* renderer, int x1, int y1, int dx, int dy, int xstep,
	int ystep, double pixel_thickness, int pxstep, int pystep
);
void DrawYThickLine(
	SDL_Renderer* renderer, int x1, int y1, int dx, int dy, int xstep,
	int ystep, double pixel_thickness, int pxstep, int pystep
...
struct Line {
	...
	void Draw(const Color& color, double pixel_thickness = 1) const {
		if (pixel_thickness <= 1) {
			...
			impl::DrawThickLine(
				static_cast<int>(a.x), static_cast<int>(a.y),
				static_cast<int>(b.x), static_cast<int>(b.y), pixel_thickness,
				color
			);
		}
	}
...
struct Capsule {
	...
	Capsule(const Line<T>& segment, T radius) :
	...
	void Draw(const Color& color, double pixel_thickness = 1) const {
		if (pixel_thickness <= 1) {
			impl::DrawCapsule(
				static_cast<int>(segment.a.x), static_cast<int>(segment.a.y),
				static_cast<int>(segment.b.x), static_cast<int>(segment.b.y),
			...
			impl::DrawThickCapsule(
				static_cast<int>(segment.a.x), static_cast<int>(segment.a.y),
				static_cast<int>(segment.b.x), static_cast<int>(segment.b.y),
				static_cast<int>(radius), pixel_thickness, color
			);
		}
	}
