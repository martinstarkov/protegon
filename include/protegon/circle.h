...
struct Circle {
	...
	void DrawSolidSliced(
		const Color& color, std::function<bool(double y_frac)> condition
	) const {
		impl::DrawSolidCircleSliced(
			static_cast<int>(center.x), static_cast<int>(center.y),
			static_cast<int>(radius), color, condition
		);
	}
};
...
struct Arc {
	...
	void Draw(const Color& color, double pixel_thickness = 1) const {
		if (pixel_thickness <= 1) {
			impl::DrawArc(
				static_cast<int>(center.x), static_cast<int>(center.y),
				static_cast<int>(radius), static_cast<double>(start_angle),
				static_cast<double>(end_angle), color
			);
		} else {
			impl::DrawThickArc(
				static_cast<int>(center.x), static_cast<int>(center.y),
				static_cast<int>(radius), static_cast<double>(start_angle),
				static_cast<double>(end_angle), pixel_thickness, color
			);
		}
	}
...
struct Ellipse {
	...
	void Draw(const Color& color, double pixel_thickness = 1) const {
		if (pixel_thickness <= 1) {
			impl::DrawEllipse(
				static_cast<int>(center.x), static_cast<int>(center.y),
				static_cast<int>(radius.x), static_cast<int>(radius.y), color
			);
		} else {
			impl::DrawThickEllipse(
				static_cast<int>(center.x), static_cast<int>(center.y),
				static_cast<int>(radius.x), static_cast<int>(radius.y),
