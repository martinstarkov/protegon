...
struct Circle {
	...
	void Draw(const Color& color, double pixel_thickness = 1) const {
		if (pixel_thickness <= 1) {
			impl::DrawCircle(
				static_cast<int>(center.x), static_cast<int>(center.y), static_cast<int>(radius),
				color
			);
		} else {
			impl::DrawThickCircle(
				static_cast<int>(center.x), static_cast<int>(center.y), static_cast<int>(radius),
				pixel_thickness, color
			);
		}
	}
	...
	void DrawSolid(const Color& color) const {
		impl::DrawSolidCircle(
			static_cast<int>(center.x), static_cast<int>(center.y), static_cast<int>(radius), color
		);
	}

	// condition should be a lambda which returns true for pixels where the
	// circle is rendered. y_frac indicates which fraction of the radius the
	// circle is rendering: y_frac is 0.0 (top of circle) to 1.0 (bottom of
	// circle), 0.5 is the center.
...
struct Arc {
	...
	void Draw(const Color& color, double pixel_thickness = 1) const {
		if (pixel_thickness <= 1) {
			impl::DrawArc(
				static_cast<int>(center.x), static_cast<int>(center.y), static_cast<int>(radius),
				static_cast<double>(start_angle), static_cast<double>(end_angle), color
			);
		} else {
			impl::DrawThickArc(
				static_cast<int>(center.x), static_cast<int>(center.y), static_cast<int>(radius),
				static_cast<double>(start_angle), static_cast<double>(end_angle), pixel_thickness,
				color
			);
		}
	}
...
struct Ellipse {
	...
	void Draw(const Color& color, double pixel_thickness = 1) const {
		if (pixel_thickness <= 1) {
			impl::DrawEllipse(
				static_cast<int>(center.x), static_cast<int>(center.y), static_cast<int>(radius.x),
				static_cast<int>(radius.y), color
			);
		} else {
			impl::DrawThickEllipse(
				static_cast<int>(center.x), static_cast<int>(center.y), static_cast<int>(radius.x),
				static_cast<int>(radius.y), pixel_thickness, color
			);
		}
	}
