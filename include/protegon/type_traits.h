...
template <typename T, typename U>
struct is_not_equals_comparable<
	T, U, std::void_t<not_equals_comparison_t<T, U>>> :
...
struct is_less_than_comparable<
	T, U, std::void_t<less_than_comparison_t<T, U>>> :
	std::is_same<less_than_comparison_t<T, U>, bool> {};

template <typename T, typename U>
using less_than_or_equal_comparison_t =
	decltype(std::declval<T&>() <= std::declval<U&>());
...
struct is_less_than_or_equal_comparable<
	T, U, std::void_t<less_than_or_equal_comparison_t<T, U>>> :
	std::is_same<less_than_or_equal_comparison_t<T, U>, bool> {};

template <typename T, typename U>
using greater_than_comparison_t =
	decltype(std::declval<T&>() > std::declval<U&>());
...
struct is_greater_than_comparable<
	T, U, std::void_t<greater_than_comparison_t<T, U>>> :
	std::is_same<greater_than_comparison_t<T, U>, bool> {};

template <typename T, typename U>
using greater_than_or_equal_comparison_t =
	decltype(std::declval<T&>() >= std::declval<U&>());
...
struct same_decayed :
	pred_base<std::is_same<
		typename std::decay<T>::type, typename std::decay<O>::type>::value> {};
...
struct both_numeric :
	both<A, B, is_numeric> {}; // Are both A and B numeric types

template <class A, class B>
struct both_floating :
	both<A, B, std::is_floating_point> {
}; // Are both A and B floating point types

template <class A, class B>
struct both_integral :
	both<A, B, std::is_integral> {}; // Are both A and B integral types

template <class A, class B>
struct both_signed :
	both<A, B, std::is_signed> {}; // Are both A and B signed types

template <class A, class B>
struct both_unsigned :
	both<A, B, std::is_unsigned> {}; // Are both A and B unsigned types
...
struct is_safe_numeric_cast :
	pred_base<
		(both_numeric<T, F>::value && std::is_floating_point<T>::value &&
		 std::is_integral<F>::value) ||
		(sizeof(T) >= sizeof(F)) ||
		(both_integral<T, F>::value &&
		 ((sizeof(T) > sizeof(F)) || (sizeof(T) == sizeof(F))) &&
...
struct is_stream_writable<
	Stream, Type,
	std::void_t<decltype(std::declval<Stream&>() << std::declval<Type>())>> :
	std::true_type {};
...
template <typename T, typename U>
inline constexpr bool is_equals_comparable_v{
	impl::is_equals_comparable<T, U>::value
};
template <typename T, typename U>
inline constexpr bool is_not_equals_comparable_v{
	impl::is_not_equals_comparable<T, U>::value
};
template <typename T, typename U>
inline constexpr bool is_less_than_comparable_v{
	impl::is_less_than_comparable<T, U>::value
};
template <typename T, typename U>
...
inline constexpr bool is_greater_than_comparable_v{
	impl::is_greater_than_comparable<T, U>::value
};
template <typename T, typename U>
...
template <typename Stream, typename Type>
inline constexpr bool is_stream_writable_v{
	impl::is_stream_writable<Stream, Type>::value
};
template <typename From, typename To>
inline constexpr bool is_safely_castable_v{
	impl::is_safely_castable<From, To>::value
};
...
template <typename T, typename U>
using not_equals_comparable =
	std::enable_if_t<is_not_equals_comparable_v<T, U>, bool>;
template <typename T, typename U>
using less_than_comparable =
	std::enable_if_t<is_less_than_comparable_v<T, U>, bool>;
template <typename T, typename U>
...
using greater_than_comparable =
	std::enable_if_t<is_greater_than_comparable_v<T, U>, bool>;
template <typename T, typename U>
...
using copy_constructible =
	std::enable_if_t<std::is_copy_constructible_v<T>, bool>;
template <typename From, typename To>
using is_safely_castable =
	std::enable_if_t<is_safely_castable_v<From, To>, bool>;
template <typename T, typename... TArgs>
using constructible = std::enable_if_t<
	std::is_constructible_v<T, TArgs...> ||
		std::is_trivially_constructible_v<T, TArgs...>,
	bool>;
template <typename Stream, typename... Types>
using stream_writable = std::enable_if_t<
	std::conjunction_v<impl::is_stream_writable<Stream, Types>...>, bool>;
template <typename Type, typename... Types>
using type =
	std::enable_if_t<std::conjunction_v<std::is_same<Type, Types>...>, bool>;
...
using is_base_of =
	std::enable_if_t<std::is_base_of_v<ParentClass, ChildClass>, bool>;
template <typename T, typename... Ts>
using is_one_of = std::enable_if_t<type_traits::is_one_of_v<T, Ts...>, bool>;
template <typename T, typename... Ts>
using is_safely_castable_to_one_of = std::enable_if_t<
	type_traits::is_safely_castable_to_one_of_v<T, Ts...>, bool>;
