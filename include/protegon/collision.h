...
[[nodiscard]] float SquareDistancePointRectangle(
	const Point<float>& a, const Rectangle<float>& b
);

[[nodiscard]] float ParallelogramArea(
...
// Page 79.
[[nodiscard]] bool RectangleRectangle(
...
// http://www.r-5.org/files/books/computers/algo-list/realtime-3d/Christer_Ericson-Real-Time_Collision_Detection-EN.pdf
// Page 88.
[[nodiscard]] bool CircleCircle(const Circle<float>& a, const Circle<float>& b);

// Source:
// http://www.r-5.org/files/books/computers/algo-list/realtime-3d/Christer_Ericson-Real-Time_Collision_Detection-EN.pdf
// Page 165-166.
[[nodiscard]] bool CircleRectangle(
	const Circle<float>& a, const Rectangle<float>& b
);

[[nodiscard]] bool PointRectangle(
	const Point<float>& a, const Rectangle<float>& b
);

[[nodiscard]] bool PointCircle(const Point<float>& a, const Circle<float>& b);

// Source:
// http://www.r-5.org/files/books/computers/algo-list/realtime-3d/Christer_Ericson-Real-Time_Collision_Detection-EN.pdf
// Page 130. (SqDistPointSegment == 0) but optimized.
[[nodiscard]] bool PointSegment(const Point<float>& a, const Segment<float>& b);

[[nodiscard]] bool SegmentRectangle(
	const Segment<float>& a, const Rectangle<float>& b
);

// Source: https://www.baeldung.com/cs/circle-line-segment-collision-detection
[[nodiscard]] bool SegmentCircle(
	const Segment<float>& a, const Circle<float>& b
);

// Source:
// https://www.geeksforgeeks.org/check-if-two-given-line-segments-intersect/
[[nodiscard]] bool SegmentSegment(
	const Segment<float>& a, const Segment<float>& b
);
...
relative_velocity -= target_velocities[i]; const CollisionManifold collision =
lambda(position, size, target_positions[i], target_sizes[i], relative_velocity);
			if (collision.occurs) {
		...
		if (collisions.size() > 0) {
			V2_double new_velocity = internal::GetNewVelocity(out_velocity,
collisions[0]); final_velocity += out_velocity * collisions[0].time;
			// Potential alternative solution to corner clipping:
			//new_origin = origin + (velocity * collisions[0].time -
velocity.Unit() * epsilon); std::vector<CollisionManifold> collisions2;
			...
			if (!new_velocity.IsZero()) {
				for (std::size_t i = 0; i < target_positions.size(); ++i) {
					...
					if (use_relative_velocity) new_relative_velocity -=
target_velocities[i]; const CollisionManifold collision = lambda(new_position,
size, target_positions[i], target_sizes[i], new_relative_velocity); if
(collision.occurs) { collisions2.push_back(collision);
					}
				}
				internal::SortCollisionTimes(collisions2);
				if (collisions2.size() > 0) {
					final_velocity += new_velocity * collisions2[0].time;
					//draw::Line(new_position, new_position + new_velocity *
collisions2[0].time, color::Red);
					//draw::Circle(new_position + new_velocity *
collisions2[0].time, size, color::Red);
					//draw::Circle(position + final_velocity, size, color::Red);
				} else {
					final_velocity += new_velocity;
					//draw::Line(new_position, new_position + new_velocity,
color::Red);
					//draw::Circle(new_position + new_velocity, size,
color::Red);
				}
			}
		} else {
