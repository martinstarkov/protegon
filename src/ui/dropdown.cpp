...
void Dropdown::SetDropdownDirection(Origin dropdown_direction) {
    Create();
    PTGN_ASSERT(dropdown_direction != Origin::Center, "Cannot set dropdown direction to be Origin::Center");
...
void Dropdown::Draw(const Rect& parent_button_rect, const LayerInfo& layer_info) {
    ...
    for (auto& button : i.buttons_) {
        ...
        button.SetRect(r);
        button.Set<ButtonProperty::LayerInfo>(layer_info);
        button.SetInternalOnActivate([this](){
            Hide();
        });
        button.Draw();
    }
