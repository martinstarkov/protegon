...
// composition / system.For example,
//	to add a border to a button you would call button.Add<Border>(line_width, / color);
// Or to add a background color : button.Add<BackgroundColor>();
// And these classes would add /
//	components to the parent button entity.For example adding BackgroundColor would add a /
//	DefaultColor,
//	HoverColor, PressedColor.If the button has a Toggleable component already,
//	adding / BackgroundColor would also add ToggledHoverColor,
//	etc.Adding a Toggleable component would check /
//		if there is a BackgroundColor and add ToggledHoverColor,
//	etc if there is.This way a button can /
...
//		TextSize, // Type: V2_float (default: unscaled text size). If either axis of the text size
//				  // is zero, it is stretched to fit the entire size of the button rectangle (along
//				  // that axis).
...
//			auto& i{ Handle::Get() };
//			auto& resource{
//				impl::GetResource(state, toggled, disabled, i.GetResource<Property>())
//			};
...
//			Button& button{
//				MapManager::Load(key, Button{ std::forward<TArgs>(constructor_args)... })
//			};
