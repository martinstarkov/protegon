...
template <typename VertexType, std::size_t VertexCount>
static auto GetVertices(
...
struct Batch {
	...
	[[nodiscard]] float GetAvailableTextureIndex(const Texture& texture) {
		PTGN_ASSERT(texture != game.renderer.white_texture_);
...
private:
	...
	std::vector<Batch>& GetLayerBatches(std::int32_t render_layer, [[maybe_unused]] float alpha) {
		// TODO: Add opaque batches back once you figure out how to do it using depth testing.
		/*
		// Transparent object.
	...
	std::pair<std::vector<std::array<VertexType, VertexCount>>&, float> GetAvailableBatch(
		...
		if (is_quad && texture != game.renderer.white_texture_) {
			PTGN_ASSERT(texture.IsValid());
	...
	static auto GetBufferInfo(Batch& batch) {
		...
		} else if constexpr (s == BatchType::Triangle) {
			return std::make_tuple(batch.triangles_, 3);
		} else if constexpr (s == BatchType::Line) {
			return std::make_tuple(batch.lines_, 2);
		} else if constexpr (s == BatchType::Circle) {
			return std::make_tuple(batch.circles_, 6);
		} else if constexpr (s == BatchType::Point) {
	...
	template <typename T>
	static void DrawVAO(const VertexArray& vao, const T& data, std::size_t index_count) {
		vao.GetVertexBuffer().SetSubData(
			data.data(), static_cast<std::uint32_t>(SizeofVector(data)), false
		);
		GLRenderer::DrawElements(vao, data.size() * index_count, false);
	...
	void FlushType(const std::vector<Batch>& batches) {
		auto vao{ GetVertexArray<T>() } vao.Bind();
		for (auto& batch : batches) {
