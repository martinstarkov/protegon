...
static SDL_bool CompileShader(GLhandleARB shader, const char *source) {
	...
	if (status == 0) {
		...
		if (!info) {
			...
			SDL_LogError(
				SDL_LOG_CATEGORY_APPLICATION, "Failed to compile shader:\n%s\n%s", source, info
			);
			SDL_free(info);
		}
		return SDL_FALSE;
...
static SDL_bool InitShaders() {
	...
	if (SDL_GL_ExtensionSupported("GL_ARB_shader_objects") &&
		...
		pglCreateShaderObjectARB =
			(PFNGLCREATESHADEROBJECTARBPROC)SDL_GL_GetProcAddress("glCreateShaderObjectARB");
		...
		pglUseProgramObjectARB =
			(PFNGLUSEPROGRAMOBJECTARBPROC)SDL_GL_GetProcAddress("glUseProgramObjectARB");
		if (pglAttachObjectARB && pglCompileShaderARB && pglCreateProgramObjectARB &&
			pglCreateShaderObjectARB && pglDeleteObjectARB && pglGetInfoLogARB &&
			pglGetObjectParameterivARB && pglGetUniformLocationARB && pglLinkProgramARB &&
			pglShaderSourceARB && pglUniform1iARB && pglUseProgramObjectARB) {
...
int TestProgram() {
	...
	window = SDL_CreateWindow(
		"Shader Demo", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 640, 480, SDL_WINDOW_OPENGL
	);
	if (!window) {
		SDL_LogError(
			SDL_LOG_CATEGORY_APPLICATION, "Unable to create OpenGL window: %s\n", SDL_GetError()
		);
		SDL_Quit();
		exit(2);
	}
	...
	if (!SDL_GL_CreateContext(window)) {
		SDL_LogError(
			SDL_LOG_CATEGORY_APPLICATION, "Unable to create OpenGL context: %s\n", SDL_GetError()
		);
		SDL_Quit();
		exit(2);
	}

	surface = SDL_LoadBMP("resources/sprites/icon.bmp");
	if (!surface) {
