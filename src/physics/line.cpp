...
void DrawThickCapsule(
	int x1, int y1, int x2, int y2, int r, double pixel_thickness, const Color& color
) {
	auto renderer{ SetDrawMode(color) };
...
void DrawThickLineImpl(
	SDL_Renderer* renderer, int x1, int y1, int x2, int y2, double pixel_thickness
) {
	int wh;
	...
	if (dx > dy) {
		DrawXThickLine(
			renderer, x1, y1, dx, dy, xstep, ystep, pixel_thickness + 1.0, pxstep, pystep
		);
	} else {
		DrawYThickLine(
			renderer, x1, y1, dx, dy, xstep, ystep, pixel_thickness + 1.0, pxstep, pystep
...
void DrawCapsuleImpl(SDL_Renderer* renderer, int x1, int y1, int x2, int y2, int r) {
	...
	const double angle{ RadToDeg(RestrictAngle2Pi(dir.Angle() + half_pi<double>)) };
	...
	// Draw edge lines.
	DrawLineImpl(renderer, x1 + tangent_r.x, y1 + tangent_r.y, x2 + tangent_r.x, y2 + tangent_r.y);
	DrawLineImpl(renderer, x1 - tangent_r.x, y1 - tangent_r.y, x2 - tangent_r.x, y2 - tangent_r.y);
...
void DrawSolidCapsuleImpl(SDL_Renderer* renderer, int x1, int y1, int x2, int y2, int r) {
	...
	// const float angle{ RadToDeg(RestrictAngle2Pi(dir.Angle<float>() +
	// half_pi<float>)) }; const int dir2{ dir.Dot(dir) };
	...
	// return;
	//} else {
	// tangent_r = static_cast<V2_int>((dir.Skewed() / std::sqrt(dir2) *
	// r).FastFloor());
	//}
	//
...
void DrawThickCapsuleImpl(
	SDL_Renderer* renderer, int x1, int y1, int x2, int y2, int r, double pixel_thickness
) {
	V2_int dir{
		V2_int{x2, y2}
		 - V2_int{x1, y1}
	...
	// Draw edge lines.
	DrawThickLineImpl(
		renderer, x1 + tangent_r.x, y1 + tangent_r.y, x2 + tangent_r.x, y2 + tangent_r.y,
		pixel_thickness
	);
	DrawThickLineImpl(
		renderer, x1 - tangent_r.x, y1 - tangent_r.y, x2 - tangent_r.x, y2 - tangent_r.y,
...
void DrawThickVerticalLineImpl(
	SDL_Renderer* renderer, int x, int y1, int y2, double pixel_thickness
) {
...
void DrawThickHorizontalLineImpl(
	SDL_Renderer* renderer, int x1, int x2, int y, double pixel_thickness
) {
	DrawThickLineImpl(renderer, x1, y, x2, y, pixel_thickness);
}

void DrawXPerpendicular(
	SDL_Renderer* renderer, int x1, int y1, int dx, int dy, int xstep, int ystep, int einit,
	int w_left, int w_right, int winit
) {
	...
	if (q == 0 && p < 2) {
		DrawPointImpl(renderer, x1, y1); // very thin lines
	}
}

void DrawYPerpendicular(
	SDL_Renderer* renderer, int x1, int y1, int dx, int dy, int xstep, int ystep, int einit,
	int w_left, int w_right, int winit
) {
	int x, y, threshold, E_diag, E_square;
...
void DrawXThickLine(
	SDL_Renderer* renderer, int x1, int y1, int dx, int dy, int xstep, int ystep,
	double pixel_thickness, int pxstep, int pystep
) {
	...
	for (p = 0; p < length; p++) {
		DrawXPerpendicular(renderer, x, y, dx, dy, pxstep, pystep, p_error, w_left, w_right, error);
		if (error >= threshold) {
			y	  = y + ystep;
			error = error + E_diag;
			if (p_error >= threshold) {
				DrawXPerpendicular(
					renderer, x, y, dx, dy, pxstep, pystep, (p_error + E_diag + E_square), w_left,
					w_right, error
				);
...
void DrawYThickLine(
	...
	for (p = 0; p < length; p++) {
		DrawYPerpendicular(renderer, x, y, dx, dy, pxstep, pystep, p_error, w_left, w_right, error);
		if (error >= threshold) {
			x	  = x + xstep;
			error = error + E_diag;
			if (p_error >= threshold) {
