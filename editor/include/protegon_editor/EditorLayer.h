...
class EditorApp {
 public:
  ...
  void ReparentEntityBefore(int entity_id, int new_parent_id,
  ...
  void AddComponentToSelected(ComponentKind kind);
  void RemoveComponentFromSelected(ComponentKind kind);
  ...
  Entity MakeEntity(int id, int parent_id, const char* name, bool transform,
                    bool sprite, bool camera, bool rigidbody, bool script);
  ...
  ComponentKind selected_component_ = ComponentKind::None;
