...
bool DrawPhysicsFeature(Target& target) {
	...
	auto apply_movement_change = [&](auto&& mutate) {
		...
		auto apply{ MakeInspectorFeatureApply(
			target, InspectorFeature::Physics, components
		) };
	...
	if (movement_open) {
		...
		if (
			...
			if (ImGui::TreeNodeEx(
				...
				const std::string jump_preview{
					selected_controller ? selected_controller->label : "None"
				};
			...
			if (Entity owner{ GetInspectorTargetEntity(target) };
				...
				if (Entity ground{ live.GetGroundEntity() }) {
					const std::string ground_name{
						ground.Has<Tag>() && !ground.Get<Tag>().value.empty()
