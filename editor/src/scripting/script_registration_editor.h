...
#define PTGN_IMPL_REGISTER_EDITOR_SCRIPT_IMPL(Type, Id, ...)                             \
	...
	){ ::ptgn::editor::RegisterScriptEditors<Type>(__VA_ARGS__) };                 \
	}
...
/// @brief Registers one or more editor definitions for a Script type.
///
/// Runtime Script metadata must be registered separately in
/// runtime/scripting/register_engine_scripts.cpp.
#define PTGN_REGISTER_SCRIPT(Type, ...)                                                  \
	__VA_OPT__(PTGN_IMPL_REGISTER_EDITOR_SCRIPT(Type, __VA_ARGS__))

/// @brief Registers an editor definition for a sequence event type.
///
/// Runtime event matching must be registered separately in
/// runtime/scripting/register_engine_scripts.cpp.
