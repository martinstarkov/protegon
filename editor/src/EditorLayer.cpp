...
void EditorApp::Run() {
	...
	while (running) {
		...
		const int fb_width = static_cast<int>(
			ImGui::GetIO().DisplaySize.x * ImGui::GetIO().DisplayFramebufferScale.x
		);
		const int fb_height = static_cast<int>(
...
SceneData& EditorApp::CurrentScene() {
...
void EditorApp::BeginRenameSceneInline(int scene_index) {
	...
	std::snprintf(rename_buffer_, sizeof(rename_buffer_), "%s", scenes_[scene_index].name.c_str());
}
...
void EditorApp::ReparentEntityBefore(int entity_id, int new_parent_id, int before_entity_id) {
