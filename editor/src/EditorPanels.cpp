#include "protegon_editor/EditorPanels.h"
...
#include "protegon_editor/EditorLayer.h"
...
bool IsImmediateNextVisibleSibling(
	EditorApp& app, int entity_id, int before_entity_id, int parent_id
) {
	const auto siblings = GetVisibleSiblingIdsForParent(app, parent_id);
...
void DrawSiblingEndDropDivider(EditorApp& app, const char* id_suffix, int parent_id) {
...
void DrawSiblingDropDivider(
	EditorApp& app, const char* id_suffix, int parent_id, int before_entity_id
) {
	if (!IsHierarchyEntityDragActive()) {
		return;
	}
...
void DrawEntityNodeRecursive(
	EditorApp& app, int entity_id, int visible_sibling_index, int visible_sibling_count
) {
...
bool DrawComponentHeader(
	EditorApp& app, const char* label, ComponentKind kind, bool non_removable
) {
...
void DrawHierarchyWindow(EditorApp& app) {
	...
	if (ImGui::BeginTabBar("SceneTabs")) {
		if (ImGui::BeginTabItem("Hierarchy")) {
			...
			std::snprintf(
				scene_name_buffer, sizeof(scene_name_buffer), "%s", app.CurrentScene().name.c_str()
			);
			...
			ImGui::InputTextWithHint(
				"##HierarchyFilter", "incl, -excl", app.hierarchy_filter_,
...
struct HasDrawComponentImpl<
	T, std::void_t<decltype(DrawComponentImpl(
		   std::declval<EditorApp&>(), std::declval<T&>(), std::declval<ComponentKind>()
	   ))>> : std::true_type {};
...
static void DrawComponentImpl(EditorApp& app, TransformComponent& transform) {
	...
	// Position
	{
		...
		ImGui::DragFloat("##PositionX", &transform.position[0], 0.1f, 0.0f, 0.0f, "%.0f");
		ImGui::EndGroup();
		...
		ImGui::SetNextItemWidth(field_width);
		ImGui::DragFloat("##PositionY", &transform.position[1], 0.1f, 0.0f, 0.0f, "%.0f");
		ImGui::EndGroup();
		...
		ImGui::SetNextItemWidth(field_width);
		ImGui::DragFloat(
			"##PositionDepth", &transform.position[2], 0.05f, -1000.0f, 1000.0f, "%.0f"
		);
		ImGui::EndGroup();
	...
	// Scale
	{
		...
		ImGui::SetNextItemWidth(field_width);
		if (ImGui::DragFloat(
				"##ScaleX", &transform.scale[0], 0.01f, -1000.0f, 1000.0f, "%.2f",
		...
		ImGui::SetNextItemWidth(field_width);
		if (ImGui::DragFloat(
				"##ScaleY", &transform.scale[1], 0.01f, -1000.0f, 1000.0f, "%.2f",
				ImGuiSliderFlags_AlwaysClamp
			)) {
...

	{
		...
		ImGui::DragFloat(
			"##Rotation", &transform.rotation, 0.1f, 0.0f, 360.0f, "%.1f deg",
			ImGuiSliderFlags_AlwaysClamp
		);
	}
}

static void DrawComponentImpl(EditorApp& app, SpriteRendererComponent& sprite_renderer) {
...
static void DrawComponentImpl(EditorApp& app, RigidBody2DComponent& rigidbody_2d) {
...
concept HasInspectorDraw =
	requires(EditorApp& app, T& component) { DrawComponentImpl(app, component); };
...
void DrawInspectorWindow(EditorApp& app) {
	...
	DrawComponent<ComponentKind::Transform>(app, entity->has_transform, entity->transform);

	DrawComponent<ComponentKind::SpriteRenderer>(
		app, entity->has_sprite_renderer, entity->sprite_renderer
	);

	DrawComponent<ComponentKind::Camera2D>(app, entity->has_camera_2d, entity->camera_2d);

	DrawComponent<ComponentKind::RigidBody2D>(app, entity->has_rigidbody_2d, entity->rigidbody_2d);
...
void DrawEngineSettingsWindow(EditorApp& app) {
	...
	ImGui::Checkbox("Use Logical Resolution", &app.presentation_.use_logical_resolution);
	...
	if (app.presentation_.use_logical_resolution) {
		...
		if (ImGui::BeginTable("##EngineSettingsTable", 2, ImGuiTableFlags_SizingFixedFit)) {
			...
			ImGui::SetNextItemWidth(field_width);
			ImGui::DragInt(
				"##LogicalWidth", &app.presentation_.logical_width, 1.0f, 1, 4096, "W: %d",
				ImGuiSliderFlags_AlwaysClamp
			);
			...
			ImGui::SetNextItemWidth(field_width);
			ImGui::DragInt(
				"##LogicalHeight", &app.presentation_.logical_height, 1.0f, 1, 2160, "H: %d",
				ImGuiSliderFlags_AlwaysClamp
			);
			...
			if (ImGui::Combo(
				...
				app.presentation_.scaling_mode = static_cast<EditorApp::ScalingMode>(scaling_mode);
			}
...
void DrawGameWindow(EditorApp& app) {
	...
	if (app.presentation_.use_logical_resolution) {
		...
		const float logical_h = static_cast<float>(app.presentation_.logical_height);
		...
		} else if (app.presentation_.scaling_mode == EditorApp::ScalingMode::Stretch) {
			view_w = avail.x;
			view_h = avail.y;
		}
	}
	...
	const ImU32 game_color = ImGui::ColorConvertFloat4ToU32(
		ImVec4(app.clear_color_[0], app.clear_color_[1], app.clear_color_[2], app.clear_color_[3])
	);
...
static ImTextureID LoadFakeThumbnailForPath(EditorApp& app, const std::string& path) {
	(void)app;
