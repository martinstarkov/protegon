...
void DrawAddComponentMenuItems(EditorApp& app) {
...
bool DrawInlineRenameField(EditorApp& app, int id, bool is_entity) {
  ImGui::PushID(id);

  const bool first_frame_of_rename =
      (is_entity && app.renaming_entity_id_ == id) ||
...
bool EntityMatchesFilter(const char* entity_name, const char* filter_text) {
  ...
  const std::string name =
      lower(entity_name ? std::string(entity_name) : std::string{});
  const std::string filter =
      filter_text ? std::string(filter_text) : std::string{};
...
bool EntityOrDescendantMatchesFilter(EditorApp& app, int entity_id) {
...
int GetDraggedEntityId() {
  const ImGuiPayload* payload = ImGui::GetDragDropPayload();
  if (!payload || !payload->IsDataType("PTGN_ENTITY") ||
...
std::vector<int> GetVisibleSiblingIdsForParent(EditorApp& app, int parent_id) {
  std::vector<int> result;
  for (const auto& e : app.CurrentScene().entities) {
    if (e.parent_id == parent_id &&
        EntityOrDescendantMatchesFilter(app, e.id)) {
...
bool IsFirstVisibleSibling(EditorApp& app, int entity_id, int parent_id) {
  const auto siblings = GetVisibleSiblingIdsForParent(app, parent_id);
  return !siblings.empty() && siblings.front() == entity_id;
}

bool IsLastVisibleSibling(EditorApp& app, int entity_id, int parent_id) {
  const auto siblings = GetVisibleSiblingIdsForParent(app, parent_id);
  return !siblings.empty() && siblings.back() == entity_id;
}

int GetVisibleSiblingIndex(EditorApp& app, int entity_id, int parent_id) {
  const auto siblings = GetVisibleSiblingIdsForParent(app, parent_id);
...
bool IsImmediateNextVisibleSibling(EditorApp& app, int entity_id,
                                   int before_entity_id, int parent_id) {
  const auto siblings = GetVisibleSiblingIdsForParent(app, parent_id);
...
void DrawRootSectionHeader(const char* label) {
  ImGui::Spacing();
  ImGui::TextUnformatted(label);
  ImGui::Separator();
}

void BeginLabeledRow(const char* label, float label_width = 90.0f) {
...
void DrawSiblingEndDropDivider(EditorApp& app, const char* id_suffix,
                               int parent_id) {
  ...
  if (ImGui::BeginDragDropTarget()) {
    if (const ImGuiPayload* payload =
            ImGui::AcceptDragDropPayload("PTGN_ENTITY")) {
      const int dropped_id = *static_cast<const int*>(payload->Data);

      if (dropped_id != parent_id &&
...
void DrawSiblingDropDivider(EditorApp& app, const char* id_suffix,
                            int parent_id, int before_entity_id) {
  if (!IsHierarchyEntityDragActive()) {
    return;
  }
  ...
  const int dragged_id = GetDraggedEntityId();
  if (dragged_id >= 0) {
    if (before_entity_id == dragged_id ||
        app.IsDescendantOf(before_entity_id, dragged_id)) {
      return;
    }
    ...
    if (IsImmediateNextVisibleSibling(app, dragged_id, before_entity_id,
                                      parent_id)) {
      return;
    }
  }
...
void DrawEntityNodeRecursive(EditorApp& app, int entity_id,
                             int visible_sibling_index,
                             int visible_sibling_count) {
  ...
  DrawSiblingDropDivider(
      app, (std::string("BeforeEntity_") + std::to_string(entity_id)).c_str(),
  ...
  if (renaming_this_entity) {
    ...
    open = ImGui::TreeNodeEx(
        reinterpret_cast<void*>(static_cast<intptr_t>(entity_id)), flags, "");
    ...
    if (ImGui::IsItemClicked(ImGuiMouseButton_Left)) {
      ...
      app.selected_component_ = entity->has_transform ? ComponentKind::Transform
                                                      : ComponentKind::None;
    }
    ...
    if (!suppress_entity_drop_target && ImGui::BeginDragDropTarget()) {
      if (const ImGuiPayload* payload =
              ImGui::AcceptDragDropPayload("PTGN_ENTITY")) {
        const int dropped_id = *static_cast<const int*>(payload->Data);
        if (dropped_id != entity_id &&
            !app.IsDescendantOf(entity_id, dropped_id)) {
  ...
  if (open) {
    for (std::size_t i = 0; i < visible_children.size(); ++i) {
      DrawEntityNodeRecursive(app, visible_children[i], static_cast<int>(i),
    ...
    if (should_show_end_children_divider) {
      DrawSiblingEndDropDivider(
          app,
          (std::string("EndChildren_") + std::to_string(entity_id)).c_str(),
          entity_id);
    }
...
bool DrawComponentHeader(EditorApp& app, const char* label, ComponentKind kind,
                         bool non_removable) {
...
void DrawDockspace(EditorApp& app) {
  ...
  app.BuildDefaultDockLayout(dockspace_id);
...
void DrawHierarchyWindow(EditorApp& app) {
  ...
  if (ImGui::BeginTabBar("SceneTabs")) {
    if (ImGui::BeginTabItem("Hierarchy")) {
      ...
      std::snprintf(scene_name_buffer, sizeof(scene_name_buffer), "%s",
                    app.CurrentScene().name.c_str());
      if (ImGui::InputText("##SceneName", scene_name_buffer,
                           sizeof(scene_name_buffer))) {
        app.CurrentScene().name = scene_name_buffer;
      }
      ...
      ImGui::InputTextWithHint("##HierarchyFilter", "incl, -excl",
                               app.hierarchy_filter_,
                               sizeof(app.hierarchy_filter_));
      ...
      for (std::size_t i = 0; i < visible_root_ids.size(); ++i) {
        DrawEntityNodeRecursive(app, visible_root_ids[i], static_cast<int>(i),
    ...
    if (ImGui::BeginTabItem("Settings")) {
      ...
      ImGui::ColorEdit4("##ClearColor", app.clear_color_);
...
void DrawScenesWindow(EditorApp& app) {
  ...
  for (int i = 0; i < static_cast<int>(app.scenes_.size()); ++i) {
    if (app.renaming_scene_index_ == i) {
    ...
    if (ImGui::Selectable(app.scenes_[i].name.c_str(), selected)) {
      ...
      app.selected_entity_id_ = app.CurrentScene().entities.empty()
                                    ? -1
                                    : app.CurrentScene().entities.front().id;
  ...
  if (ImGui::BeginPopupContextWindow(
    ...
    if (ImGui::MenuItem("Add Scene")) {
      SceneData scene;
      scene.name = "NewScene_" +
                   std::to_string(static_cast<int>(app.scenes_.size()) + 1);
      scene.entities.push_back(app.MakeEntity(app.next_entity_id_++, -1,
                                              "Main Camera", true, false, true,
                                              false, false));
...
template <typename T>
struct HasDrawComponentImpl<T,
                            std::void_t<decltype(DrawComponentImpl(
                                std::declval<EditorApp&>(), std::declval<T&>(),
                                std::declval<ComponentKind>()))>>
    : std::true_type {};
...
constexpr const char* GetComponentLabel() {
  if constexpr (Kind == ComponentKind::Transform) {
    return "Transform";
  } else if constexpr (Kind == ComponentKind::SpriteRenderer) {
    return "SpriteRenderer";
  } else if constexpr (Kind == ComponentKind::Camera2D) {
    return "Camera2D";
  } else if constexpr (Kind == ComponentKind::RigidBody2D) {
    return "RigidBody2D";
  } else if constexpr (Kind == ComponentKind::Script) {
    return "Script";
  } else {
    return "Unknown";
  }
...
static void DrawComponentImpl(EditorApp& app, TransformComponent& transform) {
  (void)app;
  ...
  // Position
  {
    ...
    ImGui::DragFloat("##PositionX", &transform.position[0], 0.1f, 0.0f, 0.0f,
                     "%.0f");
    ImGui::EndGroup();
    ...
    ImGui::SetNextItemWidth(field_width);
    ImGui::DragFloat("##PositionY", &transform.position[1], 0.1f, 0.0f, 0.0f,
                     "%.0f");
    ImGui::EndGroup();
    ...
    ImGui::SetNextItemWidth(field_width);
    ImGui::DragFloat("##PositionDepth", &transform.position[2], 0.05f, -1000.0f,
                     1000.0f, "%.0f");
    ImGui::EndGroup();
  ...
  // Scale
  {
    ...
    DrawCenteredLabel("X", field_width);
    ImGui::SetNextItemWidth(field_width);
    if (ImGui::DragFloat("##ScaleX", &transform.scale[0], 0.01f, -1000.0f,
                         1000.0f, "%.2f", ImGuiSliderFlags_AlwaysClamp)) {
      ClampScaleAwayFromZero(transform.scale[0]);
    }
    ImGui::EndGroup();
    ...
    DrawCenteredLabel("Y", field_width);
    ImGui::SetNextItemWidth(field_width);
    if (ImGui::DragFloat("##ScaleY", &transform.scale[1], 0.01f, -1000.0f,
                         1000.0f, "%.2f", ImGuiSliderFlags_AlwaysClamp)) {
      ClampScaleAwayFromZero(transform.scale[1]);
    }
    ImGui::EndGroup();
...

  {
    ...
    ImGui::DragFloat("##Rotation", &transform.rotation, 0.1f, 0.0f, 360.0f,
                     "%.1f deg", ImGuiSliderFlags_AlwaysClamp);
  }
}

static void DrawComponentImpl(EditorApp& app,
                              SpriteRendererComponent& sprite_renderer) {
  ImGui::Checkbox("Enabled##SpriteRenderer", &sprite_renderer.enabled);
  ImGui::ColorEdit4("Color", sprite_renderer.color);
  ImGui::SliderInt("Order In Layer", &sprite_renderer.order_in_layer, -20, 20);
}

static void DrawComponentImpl(EditorApp& app, Camera2DComponent& camera_2d) {
...
static void DrawComponentImpl(EditorApp& app,
                              RigidBody2DComponent& rigidbody_2d) {
  ImGui::Checkbox("Enabled##RigidBody2D", &rigidbody_2d.enabled);
...
static void DrawComponentImpl(EditorApp& app, ScriptComponent& script) {
  ImGui::Checkbox("Enabled##Script", &script.enabled);
  ImGui::InputText("Class", script.class_name, sizeof(script.class_name));
}

template <typename T>
concept HasInspectorDraw = requires(EditorApp& app, T& component) {
  DrawComponentImpl(app, component);
};

template <ComponentKind Kind, typename T>
void DrawComponent(EditorApp& app, bool has_component, T& component) {
  if (!has_component) {
    return;
  }
...
void DrawInspectorWindow(EditorApp& app) {
  ...
  DrawComponent<ComponentKind::Transform>(app, entity->has_transform,
                                          entity->transform);

  DrawComponent<ComponentKind::SpriteRenderer>(app, entity->has_sprite_renderer,
                                               entity->sprite_renderer);

  DrawComponent<ComponentKind::Camera2D>(app, entity->has_camera_2d,
                                         entity->camera_2d);

  DrawComponent<ComponentKind::RigidBody2D>(app, entity->has_rigidbody_2d,
                                            entity->rigidbody_2d);

  DrawComponent<ComponentKind::Script>(app, entity->has_script, entity->script);
...
static int FindMatchingResolutionPreset(int width, int height) {
  for (int i = 0; i < IM_ARRAYSIZE(kResolutionPresets); ++i) {
    if (kResolutionPresets[i].width == width &&
        kResolutionPresets[i].height == height) {
      return i;
    }
  }
  return -1;
...
void DrawEngineSettingsWindow(EditorApp& app) {
  ...
  ImGui::Checkbox("Use Logical Resolution",
                  &app.presentation_.use_logical_resolution);

  if (app.presentation_.use_logical_resolution) {
    if (app.presentation_.logical_width < 1)
      app.presentation_.logical_width = 320;
    if (app.presentation_.logical_height < 1)
      app.presentation_.logical_height = 180;

    constexpr float kLabelWidth = 140.0f;
    constexpr float kSpacing = 6.0f;

    if (ImGui::BeginTable("##EngineSettingsTable", 2,
      ...
      int preset_index = FindMatchingResolutionPreset(
          app.presentation_.logical_width, app.presentation_.logical_height);

      const char* preview = (preset_index >= 0)
                                ? kResolutionPresets[preset_index].label
                                : "Custom";
      ...
      if (ImGui::BeginCombo("##LogicalResolutionPreset", preview)) {
        for (int i = 0; i < IM_ARRAYSIZE(kResolutionPresets); ++i) {
          ...
          if (ImGui::Selectable(kResolutionPresets[i].label, selected)) {
            app.presentation_.logical_width = kResolutionPresets[i].width;
            app.presentation_.logical_height = kResolutionPresets[i].height;
          }
          if (selected) {
      ...
      ImGui::SetNextItemWidth(field_width);
      ImGui::DragInt("##LogicalWidth", &app.presentation_.logical_width, 1.0f,
                     1, 4096, "W: %d", ImGuiSliderFlags_AlwaysClamp);

      ImGui::SameLine(0.0f, 6.0f);

      ImGui::SetNextItemWidth(field_width);
      ImGui::DragInt("##LogicalHeight", &app.presentation_.logical_height, 1.0f,
                     1, 2160, "H: %d", ImGuiSliderFlags_AlwaysClamp);
      ...
      int scaling_mode = static_cast<int>(app.presentation_.scaling_mode);

      // full width = matches both drag ints combined
      ImGui::SetNextItemWidth(-FLT_MIN);
      if (ImGui::Combo("##ScalingMode", &scaling_mode, scaling_mode_names,
                       IM_ARRAYSIZE(scaling_mode_names))) {
        app.presentation_.scaling_mode =
            static_cast<EditorApp::ScalingMode>(scaling_mode);
      }
...
void DrawGameWindow(EditorApp& app) {
  ...
  if (app.presentation_.use_logical_resolution) {
    const float logical_w = static_cast<float>(app.presentation_.logical_width);
    const float logical_h =
        static_cast<float>(app.presentation_.logical_height);

    if (app.presentation_.scaling_mode == EditorApp::ScalingMode::Letterbox) {
    ...
    } else if (app.presentation_.scaling_mode ==
               EditorApp::ScalingMode::Stretch) {
      view_w = avail.x;
      view_h = avail.y;
    }
  }
  ...
  const ImU32 game_color = ImGui::ColorConvertFloat4ToU32(
      ImVec4(app.clear_color_[0], app.clear_color_[1], app.clear_color_[2],
             app.clear_color_[3]));
...
struct FakeAssetManager {
  ...
  FakeSceneAssetRegistry& GetOrCreateSceneRegistry(
  ...
  void AddAssetToScene(const std::string& scene_key,
    ...
    if (base_key.empty()) base_key = "asset";
  ...
  void RemoveAssetFromScene(const std::string& scene_key,
    ...
    if (asset_it->second.thumbnail != static_cast<ImTextureID>(0)) {
      GLuint tex = static_cast<GLuint>(
          static_cast<uintptr_t>(asset_it->second.thumbnail));
...
static std::string GetCurrentSceneKey(const EditorApp& app) {
  if (app.current_scene_index_ >= 0 &&
      app.current_scene_index_ < static_cast<int>(app.scenes_.size())) {
    return app.scenes_[app.current_scene_index_].name;
  }
  return "";
}
static ImTextureID LoadFakeThumbnailForPath(EditorApp& app,
  ...
  if (!src) {
    SDL_Log("SDL_ConvertSurface failed for '%s': %s", path.c_str(),
            SDL_GetError());
    return static_cast<ImTextureID>(0);
  }
  ...
  SDL_Surface* thumb =
      SDL_CreateSurface(kThumbSize, kThumbSize, SDL_PIXELFORMAT_ABGR8888);
  if (!thumb) {
  ...
  if (!SDL_FillSurfaceRect(thumb, nullptr,
                           SDL_MapSurfaceRGBA(thumb, 0, 0, 0, 0))) {
  ...
  const float scale = (src_w > 0.0f && src_h > 0.0f)
                          ? SDL_min(static_cast<float>(kThumbSize) / src_w,
  ...
  if (!SDL_BlitSurfaceScaled(src, nullptr, thumb, &dst_rect,
                             SDL_SCALEMODE_LINEAR)) {
    SDL_Log("SDL_BlitSurfaceScaled failed for '%s': %s", path.c_str(),
            SDL_GetError());
    SDL_DestroySurface(src);
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, thumb->w, thumb->h, 0, GL_RGBA,
               GL_UNSIGNED_BYTE, thumb->pixels);
...
static void SDLCALL OnImportAssetsSelected(void* userdata,
...
static void OpenImportAssetsDialog(SDL_Window* sdl_window) {
  ...
  SDL_ShowOpenFileDialog(OnImportAssetsSelected, nullptr, sdl_window, filters,
...
void DrawAssetsWindow(EditorApp& app) {
  ...
  ImGui::SameLine();
  ImGui::TextDisabled("| %d assets",
                      static_cast<int>(registry.assets_by_key.size()));
  ...
  std::stable_sort(sorted_assets.begin(), sorted_assets.end(),
                   [&](const FakeAssetInfo* a, const FakeAssetInfo* b) {
                     auto compare_name = [&](const FakeAssetInfo* x,
                                             const FakeAssetInfo* y) {
                       return x->key < y->key;
                     };

                     auto compare_type = [&](const FakeAssetInfo* x,
                                             const FakeAssetInfo* y) {
                       std::string x_ext =
                           std::filesystem::path(x->path).extension().string();
                       std::string y_ext =
                           std::filesystem::path(y->path).extension().string();
  ...
  ImGui::BeginChild("##AssetsScrollRegion", ImVec2(0.0f, 0.0f), false,
  ...
  const float tile_width =
      (avail_width - total_spacing) / static_cast<float>(column_count);

  const float preview_size = tile_width;
  const float tile_height = preview_size + kRemoveButtonHeight +
                            kKeyEditHeight + kFileTextHeight + 12.0f;

  if (ImGui::BeginTable("##AssetsGrid", column_count,
                        ImGuiTableFlags_SizingFixedFit |
    ...
    for (FakeAssetInfo* asset : sorted_assets) {
...

      {
        ...
        if (ImGui::InputText("##AssetKey", key_buffer, sizeof(key_buffer),
    ...
    if (!asset_to_remove.empty()) {
      g_fake_asset_manager.RemoveAssetFromScene(current_scene_key,
                                                asset_to_remove);
    }
    ...
    if (!asset_key_to_rename.empty() && !asset_new_key.empty()) {
      ...
      if (it != registry.assets_by_key.end()) {
        ...
        int suffix = 1;
        while (registry.assets_by_key.find(unique_key) !=
               registry.assets_by_key.end()) {
