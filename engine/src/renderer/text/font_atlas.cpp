...
FontAtlas::FontAtlas(
	Renderer& renderer, path font_path, path cache_png_path, const FontAtlasInfo& atlas_info
) {
	...
	atlas_texture_ = RendererAccessor{ renderer }.CreateTexture(
		surface.Data(), TextureDesc{ .size	 = surface.GetSize(),
...
const FontData& FontAtlas::GetFontData() const {
	return data_;
}

TextureId FontAtlas::GetAtlasTexture() const {
	return atlas_texture_;
}

V2_int FontAtlas::GetAtlasSize() const {
