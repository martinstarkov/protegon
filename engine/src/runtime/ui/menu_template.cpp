...
void SceneAction::SetContext(const std::shared_ptr<ApplicationContext>& ctx) {
	ctx_ = ctx;
...
void EnterSceneConfig(Scene& scene, const path& template_json_path) {
	...
	scene.app().scene.SwitchTo<impl::TemplateMenuScene>(start_scene, {}, start_scene, scene_json);
}
