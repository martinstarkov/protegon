...
class SceneAction {
public:
	static void Register(const std::string& name, const std::function<void(Scene&)>& action);
...
private:
	...
	static std::function<void(Scene&)> Get(
		const std::string& from_key, const json& scene_json, const std::string& action_name
	);

	using PrefixFunc =
		std::function<void(const std::string&, const json&, const std::string&, Scene&)>;

	std::unordered_map<std::size_t, std::function<void(Scene&)>> actions_;

	/// @brief Prefix string is used to compare the scene action.
