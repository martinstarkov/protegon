...
struct KDObject {
	...
	float GetCenter(KDAxis axis) const;
};
...
public:
	...
	/// @brief (Re)Build KD-tree from scratch (clears moved list).
	void Build(const std::vector<KDObject>& objects);

	// TODO:
	// In the future consider moving to a cached KD-tree where the following events will
	...
	void UpdateBoundingAABB(Entity entity, BoundingAABB aabb);

	/// @brief Insert new entity immediately (optional). Also mark as moved to ensure it's
	/// processed.
	void Insert(Entity entity, BoundingAABB aabb);

	/// @brief Remove entity immediately (mark for removal), processed at EndFrame.
	void Remove(Entity entity);

	/// @brief Should be called once per frame after all
	/// UpdateBoundingAABB()/Insert()/Remove()
	void EndFrameUpdate();

	/// @brief Note: If region is a bounding volume inside of the KD-tree, Query will return that
	/// region entity as well (in other words, you must check for self collisions).
	std::vector<Entity> Query(BoundingAABB region) const;

	std::vector<Entity> Query(V2_float point) const;

	/// @param entity passed to avoid raycasting against itself.
	std::vector<Entity> Raycast(Entity entity, V2_float dir, BoundingAABB aabb) const;

	/// @param entity passed to avoid raycasting against itself.
	Entity RaycastFirst(Entity entity, V2_float dir, BoundingAABB aabb) const;
