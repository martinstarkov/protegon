...
class TextSystem {
public:
	[[nodiscard]] TextLayout BuildLayout(TextLayoutRequest request);
	...
	[[nodiscard]] TextMeasurement Measure(TextLayoutRequest request);
...
private:
	...
	[[nodiscard]] static std::vector<RichTextToken> Tokenize(
		StyledText& styled_text, float global_shrink
	);
	[[nodiscard]] static float MeasureLineHeight(TextRunStyle& style);
	...
	[[nodiscard]] static float FindBestShrinkScale(StyledText& styled_text, TextBox box);
	...
	[[nodiscard]] static std::optional<ResolvedGlyph> ResolveGlyph(
		TextRun& run, uint32_t codepoint, uint32_t next_codepoint, size_t source_run_index,
		size_t source_codepoint_index, float global_shrink
	);
