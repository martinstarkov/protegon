...
public:
	...
	Chunk(const json& j, Scene& scene);
	...
	/// @return True if the chunk has changed from its base generated state and should be
	/// serialized, false otherwise.
	[[nodiscard]] bool HasChanged() const;

	/// @brief Flag the chunk as changed which will lead to it being serialized on the next chunk
	/// manager update.
	void FlagAsChanged(bool changed = true);

	[[nodiscard]] json Serialize() const;

	void Deserialize(const json& j, Scene& scene);
