...
Chunk::Chunk(const json& j, Scene& scene) {
	Deserialize(j, scene);
}
...
Entity NoiseLayer::GetEntity(V2_int tile_coordinate, V2_int tile_size) const {
	if (!callback) {
		return {};
	}
	float noise_value{
	...
	for (int i{ min.x }; i < max.x; i++) {
		for (int j{ min.y }; j < max.y; j++) {
			...
			if (auto cache_it{ chunk_cache.find(coordinate) }; cache_it != chunk_cache.end()) {
				chunks.try_emplace(coordinate, cache_it->second, scene);
				continue;
			}
