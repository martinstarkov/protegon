...
bool AStarGrid::SetObstacle(V2_int coordinate, bool obstacle) {
	if (Has(coordinate)) {
...
bool AStarGrid::IsObstacle(V2_int coordinate) const {
	return Has(coordinate) && Get(coordinate).obstacle;
}

bool AStarGrid::IsVisited(V2_int coordinate) const {
...
int AStarGrid::FindWaypointIndex(const std::deque<V2_int>& waypoints, V2_int position) {
