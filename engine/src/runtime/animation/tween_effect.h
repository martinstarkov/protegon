...
EffectObject<TComponent>& GetTween(Entity entity) {
	...
	if (!entity.Has<EffectObject<TComponent>>()) {
		tween = &entity.Add<EffectObject<TComponent>>(CreateTween(entity.GetScene()));
 ...
 * @param target_angle The angle (in radians) to rotate the entity to. Positive clockwise,
 negative
 * counter-clockwise.
...
/**
 * @brief Fades out the specified entity over a given duration. If the object already has a tint
 of
 ...
 *
 * The bounce starts at the entity position (or previously queued bounce end point), approaches
 the
 ...
 *
 * Similar to a regular bounce, the symmetrical bounce starts at the entity position (or
 previously
