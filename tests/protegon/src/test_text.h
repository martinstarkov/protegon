...
bool TestText() {
	...
	Text test010{
		font, content, {0, 0, 255, 50},
		  FontStyle::Normal, FontRenderMode::Blended
	};
	...
	game.RepeatUntilQuit([&]() {
		...
		test002.Draw(Rectangle<int>{
			{			  0, ystride * 0},
			{window_size.x,		 ysize}
		 });
		test003.Draw(Rectangle<int>{
			{0, ystride * 1},
			text_size
		  }
		); // Actual size needed to render font without stretching.
		test004.Draw(Rectangle<int>{
			{			  0, ystride * 2},
			{window_size.x,		 ysize}
		 });
		test005.Draw(Rectangle<int>{
			{			  0, ystride * 3},
			{window_size.x,		 ysize}
		 });
		test006.Draw(Rectangle<int>{
			{			  0, ystride * 4},
			{window_size.x,		 ysize}
		 });
		test007.Draw(Rectangle<int>{
			{			  0, ystride * 5},
			{window_size.x,		 ysize}
		 });
		test008.Draw(Rectangle<int>{
			{			  0, ystride * 6},
			{window_size.x,		 ysize}
		 });
		test009.Draw(Rectangle<int>{
			{			  0, ystride * 7},
			{window_size.x,		 ysize}
		 });
		test010.Draw(Rectangle<int>{
			{			  0, ystride * 8},
			{window_size.x,		 ysize}
		 });
		test011.Draw(Rectangle<int>{
			{			  0, ystride * 9},
			{window_size.x,		 ysize}
		 });
		test012.Draw(Rectangle<int>{
			{			  0, ystride * 10},
			 {window_size.x,		  ysize}
		  });
