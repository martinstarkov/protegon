		...
		for (tile.x = 0; tile.x < tile_count.x; ++tile.x) {
			for (tile.y = 0; tile.y < tile_count.y; ++tile.y) {
				...
				if (draw_box) {
					...
					if (rng.RandomInt(0, 2) == 1) {
						entity = CreateBox(absolute_position, size, *manager, "./resources/textures/box.png");
					} else if (rng.RandomInt(0, 5) == 1) {
						entity = CreateBox(absolute_position, size, *manager, "./resources/textures/box2.png");
					} else if (rng.RandomInt(0, 5) == 1) {
						entity = CreateBox(absolute_position, size, *manager, "./resources/textures/box3.png");
					} else if (rng.RandomInt(0, 5) == 1) {
						entity = CreateBox(absolute_position, size, *manager, "./resources/textures/box4.png");
					} else if (rng.RandomInt(0, 5) == 1) {
						entity = CreateBox(absolute_position, size, *manager, "./resources/textures/tree.png");
					}
				}
			}
		}
