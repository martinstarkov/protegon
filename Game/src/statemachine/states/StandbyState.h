...
class StandbyState : public engine::State {
	...
	virtual void Update() override final {
		...
		auto players = manager->GetComponentTuple<TransformComponent, PlayerController>();
		ecs::Entity closest_player = ecs::null;
		auto closest_distance = tower.range;
		for (auto [entity, transform, player] : players) {
			double distance = (tower_transform.position - transform.position).Magnitude();
		...
		if (tower.projectiles > 0) {
			if (closest_player.IsAlive()) {
				...
				DebugDisplay::circles().emplace_back(tower_transform.position, tower.range, engine::GREEN);
				...
				DebugDisplay::circles().emplace_back(tower_transform.position, tower.range, engine::ORANGE);
