...
Window::Window(const char* title, const V2_int& position, const V2_int& size, std::uint32_t flags) : window{ SDL_CreateWindow(title, position.x, position.y, size.x, size.y, flags) } {
	if (!IsValid()) {
